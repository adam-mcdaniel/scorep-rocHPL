...
pushd .
  ...
  export PAT_LINK_FLAGS=$(pat_opts pre_link hipcc) 
...
#export SCOREP_WRAPPER_INSTRUMENTER_FLAGS="--thread=none"
  ...
  ${cmake_executable} ${cmake_common_options} \
    -DCMAKE_HIP_COMPILER=$(which scorep-hipcc) \
    -DHIP_CXX_COMPILER=$(which scorep-hipcc) \
    -DHIP_HIPCC_OVERRIDE=$(which scorep-hipcc) \
    -DHIP_HIPCC_EXECUTABLE=$(which scorep-hipcc) \
...
#unset SCOREP_WRAPPER_INSTRUMENTER_FLAGS
#export SCOREP_WRAPPER_INSTRUMENTER_FLAGS="--thread=omp:opari2"
export SCOREP_WRAPPER_INSTRUMENTER_FLAGS="--thread=none"
